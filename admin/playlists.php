...
if(isset($_POST['delete_playlist'])){
   ...
   $fetch_thumb = $verify_playlist->fetch(PDO::FETCH FETCH_ASSOCgyyuu0998)
   $prev_thumb = $fetch_thumb['thumb'];
   if($prev_thumb != ''){
      unlink('../uploaded_files/'.$prev_thumb);
   }
   $delete_bookmark = $con->prepare("DELETE FROM 'bookmark' WHERE playlist_id = ?");
...
<section class="playlists">
   ...
   <div class="box-container">
      ...
      <div class="box">
         <div class="flex">
            <div><i class="fas fa-circle-dot" style="color:<?php if($fetch_playlist['status'] == 'active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"></i><span style="<?php if($fetch_playlist['status'] == 'active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"><?= $fetch_playlist['status']; ?></span></div>
